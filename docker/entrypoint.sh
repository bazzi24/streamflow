...
# -----------------------------
# usage and comand line arguments
# -----------------------------

function usage() {
    echo "Usage $0 [--disable-datasync] [--worker=<num>] [--host-id=<string>]"
    echo 
    echo "  --disable-datasync              Disables synchronzation of datasaource worker."
    echo "  --workers=<num>                 Number of task excutors to run (if range is not used)."
    echo "  --host-id=<string>              Unique ID for the host (defaults to \`hostname\`)." 
    echo "  --init-superuser                Initializes the super user."
    echo 
    echo "Examples:"
    echo "  $0 --disable-datasync"
...
INIT_SUPERUSER_ARGS=""
WORKER=1
PY_SCRIPT_PATH="kafkaStream/producer_market_data.py"
CHANNLES=("X-TRADE:ALL" "X-QUOTE:ALL" "R:ALL" "MI:ALL" "F:ALL")

run_producer() {
    local channel=$1
    local term_num=$2

    echo "Starting task $term_num: $channel"
    gnome-terminal --title="Producer-$channel" -- bash -c "
        trap '
            ...
            echo \"Producer $channel was manually stopped (Ctrl+C)\" | mail -s \"Producer $channel stopped manually\" dinhkhoa288@gmail.com;
            exit 130
        ' SIGINT
        ...
        if grep -q 'Connection to remote host was lost' \"\$LOG_FILE\" || [ \$EXIT_CODE -ne 0 ]; then
            ...
            echo -e \"Producer $channel has stopped or lost connection.\n\nCheck log: \$LOG_FILE\" | mail -s \"Producer $channel stopped (error)\" dinhkhoa288@gmail.com
        fi
