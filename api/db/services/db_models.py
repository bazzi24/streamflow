...
class MajorShareHolder(DatabaseModel):
    idMajor = AutoField(primary_key=True)
    
    idSymbol = ForeignKeyField(
        Corporation,
        backref="idSymbol",
        related_name="idSymmbol"
    )
    
    major = CharField(max_length=100, null=True, help_text="major corporation", index=True)
    
    
    class Meta:
        db_table = "majorShareHolder"
        
class SymbolStatus(DatabaseModel):
    idSymbolStatus = AutoField(primary_key=True)
    idSymbol = CharField(max_length=100, null=True, help_text="id Symbol", index=True)
    idMarket = CharField(max_length=100, null=True, help_text="id Market", index=True)
    status = CharField(max_length=25, null=True, help_text="status symbol", index=True)
