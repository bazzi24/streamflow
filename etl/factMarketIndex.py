...
def fact_marketindex():
    ...
    # dim.index renamed to dim.market_index (index is a reserved keyword in MySQL)
    index_dw_df    = spark.read.jdbc(url=dw_db_url, table="dim.market_index",   properties=dw_db_properties)
    ...
    existing_index_df = spark.read.jdbc(url=dw_db_url,
                                        table="warehouse.fact.marketindex",
                                        properties=dw_db_properties
                                        ).select("tradingdate_key", "time_key", "index_key")
