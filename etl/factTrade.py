...
def fact_trade():
    ...
    # MySQL syntax: DATE() cast and DATE_SUB() instead of PostgreSQL ::date and INTERVAL '5 day'
    querytrade = """
    (
        SELECT *
        FROM data.data_trade
        WHERE DATE(trading_date) >= DATE_SUB(CURRENT_DATE, INTERVAL 5 DAY)
    ) AS recent_trades
    """
    ...
    # Composite PK: (tradingdate_key, time_key, symbol_key, exchange_key, trading_session_key)
    ...
    pk_cols = ["tradingdate_key", "time_key", "symbol_key", "exchange_key", "trading_session_key"]
    ...
    for date_key in date_list:
        ...
        # Backtick-quote `change` because it is a reserved keyword in MySQL
        query = f"""
        (
            SELECT tradingdate_key, time_key, symbol_key, exchange_key, trading_session_key
            FROM warehouse.fact.stocktrade
        ...
        # left_anti on full composite PK: deduplicate ticks already written
        new_trade_df = df_day.join(existing_trade_df,
                                   on=pk_cols,
                                   how="left_anti")
